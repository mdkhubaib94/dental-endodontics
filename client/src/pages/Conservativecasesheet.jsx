...
const ConservativeCaseSheet = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllergies = async (attempt = 0) => {
      ...
      try {
        const res = await fetch(`/api/patient-details/by-patient-id/${encodeURIComponent(patientId)}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchGeneralCase = async (patientId) => {
      try {
        ...
        const res = await fetch(`/api/general/patient/${encodeURIComponent(patientId)}`, { headers });
  ...
  formData.append('treatmentPictures', JSON.stringify(treatmentPictures || []));
      ...
      const token = localStorage.getItem('token');
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await fetch('/api/conservative/save', { method: 'POST', body: formData, headers });
      ...
      <div className="login-box">
        ...
        <div style={{ maxWidth: 820, margin: '0 auto', textAlign: 'left' }}>
          ...
          <div className="input-group">
           ...
           <div className="input-group">
                <label style={{ alignSelf: 'center', color: '#e6eefc' }}>
                  Signature <span style={{ color: '#ffb4b4' }}>*</span>:&nbsp;
                  <input type="file" accept="image/*" onChange={onSignatureChange} required aria-required="true" style={{ display: 'inline-block' }} />
                </label>
          </div>
