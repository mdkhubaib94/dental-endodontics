...
import { saveSpecialistCase, ServiceError } from '../services/caseService.js';
...
router.post('/save', auth, requireRole(['doctor','chief','pg']), upload.single('digitalSignature'), async (req, res) => {
  try {
    ...
    // Build payload; attach digitalSignature buffer if present
    if (req.file) processedBody.digitalSignature = { data: req.file.buffer, contentType: req.file.mimetype, fileName: req.file.originalname };

    const result = await saveSpecialistCase({ Model: ConservativeCase, payload: processedBody, user: req.user, departmentLabel: 'Conservative Dentistry and Endodontics' });
